...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = f'Привет! Купи ' \
                                  f'{sessionStorage[user_id]["animals"][sessionStorage[user_id]["animal_id"]]}а!'
        # Получим подсказки
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        res['response'][
            'text'] = f'{sessionStorage[user_id]["animals"][sessionStorage[user_id]["animal_id"]].capitalize()}а' \
                      f' можно найти на Яндекс.Маркете!'
        if sessionStorage[user_id]['animal_id']:
            res['response']['end_session'] = True
            return
        sessionStorage[user_id]['animal_id'] = 1

    # Если нет, то убеждаем его купить слона!
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}'," \
        f" а ты купи {sessionStorage[user_id]['animals'][sessionStorage[user_id]['animal_id']]}а!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={session['animals'][session['animal_id']]}",
            "hide": True
        })
